...
export const BLOG_SLUG: Record<Locale, string> = {
  en: "blog",
  de: "ratgeber",
  it: "articoli",
  es: "articulos",
...
export const routes = {
  ...
  blog: (locale: Locale) => join(locale, BLOG_SLUG[locale]),
  category: (locale: Locale, category: Pick<Category, "slug">) =>
    join(locale, category.slug),
  ...
  article: (
    locale: Locale,
    article: Pick<ArticleWithMeta, "slug" | "categoryKey">,
  ) => {
  ...
  app: (locale: Locale, app: Pick<AppProduct, "slug">) =>
    join(locale, APPS_SLUG[locale], app.slug),
  legal: (locale: Locale, key: LegalKey) =>
    join(locale, getLegalPage(locale, key).slug),
} as const;
...
export function resolveRoute(
  ...
  if (first === BLOG_SLUG[locale] && !second) return { kind: "blog-index" };
...
export function alternatesFor(
  route: ResolvedRoute | "home" | "search",
): Alternates {
  ...
  switch (route.kind) {
    case "blog-index":
      return everyLocale((locale) => routes.blog(locale));
    case "category":
    case "week-index": {
    ...
    case "week": {
      ...
      return everyLocale(
        (locale) =>
          `/${locale}/${slugs[locale]}/${weekSlug(locale, route.page.week)}`,
      );
    }
    case "article": {
      ...
      for (const locale of LOCALES) {
        ...
        if (slug)
          alternates[locale] = `/${locale}/${categorySlugs[locale]}/${slug}`;
      }
      return alternates;
    }
    case "apps-index":
    ...
    case "app": {
      ...
      return everyLocale(
        (locale) => `/${locale}/${APPS_SLUG[locale]}/${slugs[locale]}`,
      );
    }
    case "legal": {
...
export function resolveHref(locale: Locale, href: string): string {
  ...
  if (href.startsWith("category:"))
    return routes.categoryByKey(locale, href.slice(9));
  ...
  if (href.startsWith("week:"))
    return routes.week(locale, Number(href.slice(5)));

...
import { alternatesFor, resolveRoute, routes, type ResolvedRoute } from '@/lib/routes';
...
export function generateStaticParams() {
  ...
  for (const locale of LOCALES) {
    for (const category of listCategories(locale)) {
      ...
      if (category.isWeekIndex) {
        for (let week = 1; week <= TOTAL_WEEKS; week += 1) {
          params.push({ locale, path: [category.slug, weekSlug(locale, week)] });
        }
      }
    }
    ...
    for (const article of listArticles(locale)) {
      ...
      if (category) params.push({ locale, path: [category.slug, article.slug] });
    }
...
export default async function ContentPage({ params }: Props) {
  ...
  switch (route.kind) {
    case 'category':
      return (
        <>
          <JsonLd
            data={graph([
              breadcrumbSchema([home, { name: route.category.title, path: url }]),
            ])}
          />
    ...
    case 'week-index':
      return (
        <>
          ...
          />
          <WeekIndexView locale={locale} dict={dict} category={route.category} />
        </>
      );
        ...
        <>
          <JsonLd
            data={graph([
              breadcrumbSchema([
                home,
                { name: route.category.title, path: routes.category(locale, route.category) },
          ...
          />
          <WeekView locale={locale} dict={dict} page={route.page} category={route.category} />
        </>
      );
    ...
    case 'apps-index':
      return (
        <>
          <JsonLd data={graph([breadcrumbSchema([home, { name: dict.apps.title, path: url }])])} />
    ...
    case 'legal':
      return (
        <>
          <JsonLd
            data={graph([breadcrumbSchema([home, { name: route.page.title, path: url }])])}
          />
